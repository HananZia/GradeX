...
from gradex_app.models import Subjects, SessionYearModel, Students, Attendance, AttendanceReport, \
...
def save_attendance_data(request):
    ...
    if not student_ids:
        return JsonResponse({"status": "error", "message": "No student_ids provided"})

    try:
        json_sstudent = json.loads(student_ids)
    except json.JSONDecodeError:
        return JsonResponse({"status": "error", "message": "Malformed JSON in student_ids"})
